...
function showInstructions() {
    instr.innerText = 'There should be only one source cell(Yellow cell). Grey cell represents the shelved cells. All the pickup cell(Blue cell) should share an edge with grey cell.';
}
...
function makePostRequest(set1, set2) {
    ...
    for (var i = 0; i < tempArr1.length; i++) {
        var a = Number(tempArr1[i][0]);
        var b = Number(tempArr1[i][2]);
        arr1.push({ x: a, y: b });
    }
    var tempArr2 = [...set2];
    var a = Number(tempArr2[0][0]);
    var b = Number(tempArr2[0][2]);
    var arr2 = { x: a, y: b };
    var postData = {
        source: arr2,
        items: arr1
    ...
    const proxyurl = "https://cors-anywhere.herokuapp.com/";
    var postUrl = 'https://warehouse-path-optimisation.herokuapp.com/' + wID + '/find-pick-path';
